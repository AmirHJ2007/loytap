...
// Card colour and logo aren't picked at registration any more — every new
// café starts on this same near-black default, and the owner adjusts both
// from their dashboard afterward (see the first-visit tip card there),
// rather than being asked to before they have even seen the product.
const cAccentSel = "#171717";
...
async function createCafe() {
  ...
  try {
    ...
    // Consumed once by the dashboard to show the "adjust your rewards and
    // card look" tip card on the very first load after registration — see
    // OWNER_FIRST_VISIT_TIP in owner.page.js.
    try { localStorage.setItem("loytap_owner_first_visit", "1"); } catch (_) {}
    $("stepCreateOtp").hidden = true;
