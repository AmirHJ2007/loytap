...
try{if(localStorage.getItem('loytap_signed_in')!=='1'){var _t=new URLSearchParams(location.search).get('t');if(_t){localStorage.setItem('reloy_pending_tap',JSON.stringify({code:_t,at:Date.now()}));location.replace('auth.html');}else if(location.hostname==='reloy.ir'||location.hostname==='www.reloy.ir'){location.replace('/business');}else{location.replace('auth.html');}}}catch(e){}
